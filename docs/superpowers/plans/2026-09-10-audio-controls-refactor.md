...
- **Everything added is in English** — code, comments, script names, asset names, new docs, log strings. Existing Spanish stays untouched unless a task says otherwise (2026-09-09 hard rule).
  - **Known defect in this document, corrected 2026-09-11:** many code samples below write `Debug.Log` / `LogWarning` / `LogError` message text in **Spanish**. That contradicts the rule on this very line, and the rule wins — a Task 9 implementer caught it. Write every new log message in **English**, even where the sample shows Spanish, and do not treat the deviation as a transcription error. The samples were written to match the surrounding pre-2026-09-09 Spanish logs; that is precisely the "match the surrounding language" habit the hard rule exists to stop. Log text is code you are adding. The 17 Spanish strings already written into `SceneCatalog.cs`, `AudioBank.cs`, `ControlsDiagramSettings.cs`, `SceneCatalogValidator.cs`, `AudioBankBuilder.cs` and `AudioIdGenerator.cs` were translated in a dedicated fix pass. Keep the `[ClassName]` prefix, the interpolated values, and the guidance content identical — only the language changes.
