...
- **`PlayerView.cs`**: nueva anim `RidingPage` (constante `ANIMATION_RIDING_PAGE`) con el mismo patrón de fallback-con-warning que `PullSolapasReverse`: si el skeleton todavía no la trae, cae a `Idle` sin romper. `ForceFacing(bool)` fija el flip explícito (a diferencia de `SetFacing`, que sólo reacciona a input crudo).
...
**Pendiente**: la anim `RidingPage` la tiene que exportar la animadora del skeleton de Kami (Atlas 8). Hasta entonces el warning de fallback es el comportamiento esperado. Verificar en el editor: el signo de `faceRight` pasado como `isNext` (puede quedar invertido, como ya pasa con `SetFacing`), y que `HojaMaster_Rev` tenga la cadena de huesos con el mismo nombre `PN000pageJoint30` (si no, el warning de `Hoja.cs` avisa y Kami simplemente no se engancha en ese sentido, sin romper el flujo).
