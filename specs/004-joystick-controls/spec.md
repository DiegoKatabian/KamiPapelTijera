...
**Note**: This mapping differs from standard Xbox conventions to match Kami's design:
- **A for attack** (cut scissors) is iconic and learnable by 4-year-olds
- **B for jump** feels natural after A
- **L1 for sprint** (shoulder = "push harder")
- **Y for interact** (other hand, natural grouping with movement)

## User Stories & Testing *(mandatory)*

### User Story 1 - Gamepad Attack (Priority: P1)

Player can attack with gamepad button X and it behaves identically to left-click.

**Why P1**: core mechanic — without attack, combat is broken.

**Independent Test**: hold left stick neutral, press X repeatedly. Confirm Kami plays attack anim, scissors fire, hitbox activates. Combine with movement (hold left stick, press X) and confirm `AttackMOVE` anim plays.

**Acceptance Scenarios**:

1. **Given** Kami standing idle, **When** press X, **Then** `Attack` anim plays, scissors fire, hitbox is active for ~0.3s.
2. **Given** Kami moving (left stick held), **When** press X, **Then** `AttackMOVE` anim plays (no leg keyframing), scissors still fire.
3. **Given** Kami in air, **When** press X, **Then** `AttackMOVE` plays and hitbox fires (aerial attack works).
...
Player can jump with B and sprint with L1, both working identically to keyboard.

**Why P1**: core mobility — jump is required for level design.

**Independent Test**: press B in sequence from ground, confirm jump arc. Hold L1 while moving, confirm sprint particles + velocity increase + Run anim.

**Acceptance Scenarios**:

1. **Given** Kami on ground, **When** press B, **Then** Kami jumps (velocity applied, `jump` anim plays).
2. **Given** Kami in air after pressing B, **When** release B, **Then** Kami falls without jumping again.
...
### User Story 3 - Gamepad Interact (Priority: P1)

Player can interact (talk to NPCs, open flaps, trigger solapas) with Y button, showing dialogue and triggering events same as E key.

**Why P1**: story progression blocked without interact.

**Independent Test**: stand in NPC trigger, press Y. Confirm dialogue opens with correct speaker name, portrait, text. Press Y to advance lines. Confirm quest tracking updates.

**Acceptance Scenarios**:

1. **Given** within an interact trigger (NPC, solapa, quest trigger), **When** press Y, **Then** `EventManager.OnPlayerPressedE` fires (exact same event).
2. **Given** dialogue open, **When** press Y, **Then** advance to next dialogue line (same as E).
3. **Given** solapa trigger active, **When** press Y, **Then** `TriggerSolapa.Interact()` fires, solapa opens/closes.

---

### User Story 4 - Origami with Left Stick (Generous, Priority: P2)
...
### User Story 5 - Dynamic Input Prompts (Priority: P2)
...
1. **Given** gamepad is connected and active, **When** a dialogue/tooltip with `{INPUT:interact}` placeholder renders, **Then** it displays gamepad button Y icon (animated pulse/bounce) instead of "E".
...
- **FR-001**: `PlayerInputs` struct and `PlayerController.CheckControls()` MUST read gamepad button inputs (X for attack, B for jump, L1 for sprint, Y for interact) without breaking keyboard equivalents.
...
- **FR-003**: `OrigamiRoute.SetImagePosition()` MUST accept left-stick input (`Input.GetAxis("RightHorizontal/Vertical")` mapped as mouse position within canvas space). Note: uses same axis as movement (left stick), not right stick — requires custom mapping.
...
- **FR-007**: `DialogueManager` MUST support placeholder tags in localized strings: `{INPUT:interact}`, `{INPUT:jump}`, `{INPUT:attack}`, `{INPUT:sprint}`. At runtime, replace with TMP rich text `<sprite name=button_Y>` (or fallback text "Y").
...
- **InputPromptSystem.cs** (new): singleton that detects active input device, provides `GetPromptText(inputAction)` → returns "E" or `<sprite name=button_Y>` + fallback text.
...
InteractPrompt: "Press {INPUT:interact} to talk"
```
...
- **SC-006**: Coexistence test: with both keyboard and gamepad active, prompts show both (e.g., "E / [Y]"). Removing gamepad doesn't break prompts.
...
- **TextMesh Pro integration**: import atlas into TMP Material, register sprites with names (`button_A`, `button_B`, `button_X`, `button_Y`, `button_LB`, etc.).
...
- If no gamepad icon font is available, `InputPromptSystem.GetPromptText()` returns fallback: "Y" (text) instead of `<sprite name=button_Y>`.
